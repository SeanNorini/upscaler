...
class SeamBlending(torch.nn.Module):
    def __init__(self, x_shape, scale, offset, tile_size, blend_size):
        ...
        C, H, W = x_shape
        config = SeamBlending.create_config(
            (H, W), scale, offset, tile_size, blend_size
        )
        pixels = torch.zeros(
            (C, config["y_buffer_h"], config["y_buffer_w"]), dtype=torch.float32
        )

        weights = torch.zeros(
            (C, config["y_buffer_h"], config["y_buffer_w"]), dtype=torch.float32
        )
        blend_filter = SeamBlending.create_blend_filter(
            scale, offset, tile_size, blend_size, C
        )
    ...
    def tiled_render(
        ...
        tile_size = model.find_valid_tile_size()
        ...
        minibatch = torch.zeros(
            (batch_size, C, tile_size, tile_size), device=x.device, dtype=x.dtype
        )
    ...
    def create_blend_filter(scale, offset, tile_size, blend_size, out_channels):
        ...
        x = torch.ones(
            (out_channels, inner_tile_size, inner_tile_size), dtype=torch.float32
        )
    ...
    @staticmethod
    def update(output_x, blend_filter, step_size, pixels, weights, i, j):
